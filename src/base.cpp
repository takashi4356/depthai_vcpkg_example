...
#include <time.h>
  ...
  private:
    ...
    Mat tmp;           // 一時画像
    Mat bin_img;       // 二値化画像
    Mat gray_img_tmp;  // グレースケール画像一時データ
    Mat gray_img_tmp2; // グレースケール画像一時データ2
    Mat gray_img_tmp3; // グレースケール画像一時データ3

    double maxVal;           // 最大値
    double max_match = 0;    // テンプレートマッチング最大値
    float best_v_filter = 0; // 最適なしきい値
    cv::Point max_pt;        // 最大値の座標

    int roiCnt = 0;         // 輪郭の数
    Point front_r_temp;     // ベース右前端の一時座標
    Point front_l_temp;     // ベース左前端の一時座標
    Point back_r_temp;      // ベース右後端の一時座標
    Point back_l_temp;      // ベース左後端の一時座標
    Point base_temp;        // ベース基底座標算出ワーク
    char value_c[256];      // 汎用文字列バッファ
    Point2f center_temp;    // ボール中心座標（画面上）
    Point2f center_temp_cm; // ボール中心座標（cm換算）

    Mat clip;                  // 切り出し画像
    double def_v_filter = 140; // デフォルトしきい値
    int old_base_dis = 0;      // 前回のベース距離
    float c_enkeido;           // 円形度の閾値
    float enkeido;             // 円形度
    float ball_r_min;          // ボール半径最小値
    float ball_r_max;          // ボール半径最大値
    float ball_size;           // ボールサイズチェック用
    int ball_size_b;           // ボールサイズチェック用
    int max_ball_area = 880;   // ボール面積最大値
    int mini_ball_area = 25;   // ボール面積最小値
    int c_max_ball_area;       // 設定ファイルからの最大面積
    int c_mini_ball_area;      // 設定ファイルからの最小面積
    ...
    struct ball_noise
    {
        ...
        int depth;     // ノイズの深度値
        int cnt;       // ノイズ検出回数
        int cnt_nodep; // 深度なしノイズ検出回数
        int camera_n;  // カメラ番号
        uint64_t time; // 検出時刻
    };
    ball_noise ballnoise[c_ball_noise_i_max]; // ボールノイズテーブル
  ...
  public:
    ...
    bool camera_yaw_mutch = false; // カメラの傾き角度がマッチしたか
    float c_camera_high;
    ...
    Base() // コンストラクタ
    {
        ...
        camera.vertical_fov = 49.0f;   // 垂直視野角(main側から再設定)
        camera.horizontal_fov = 72.0f; // 水平視野角(main側から再設定)
    ...
    */
    ...
    // 外部関数: ボールノイズチェック
    bool checkBallNoise(const cv::Point &center_temp, int depth, int *noise_table_no, int camera_n, int ball_detection_phase)
    {
        ...
        for (int i_ballnoise = 0; i_ballnoise < c_ball_noise_i_max; i_ballnoise++)
        {
            ...
            if (noise.time == 0)
                continue;
            if ((noise.time + out_time) < get_m_sec() && (ball_detection_phase == far_away || ball_detection_phase == invalid))
            {
                noise.x = 0;
                noise.y = 0;
                noise.depth = 0;
                ...
                noise.cnt = 0;
                noise.camera_n = 0;
                LOG_DEBUG.printf("noiseテーブルクリア(noise#:%d(SEC:%d/%d))", i_ballnoise, noise.time, get_m_sec());
                noise.time = 0;
            }
            ...
            if (isMatch)
            {
                ...
                noise.time = get_m_sec();                                                         //ノイズ検知時間の更新
        ...
        if (!ballnoisecheck_rec)
        {
            ...
            for (int i_ballnoise = 0; i_ballnoise <= c_ball_noise_i_max; i_ballnoise++)
            {
                ...
                if (noise.time == 0)
                {
                    noise = {center_temp.x, center_temp.y, depth, 0, 1, camera_n, get_m_sec()}; // 新しい位置を記録
                    *noise_table_no = i_ballnoise;
            ...
            oldest_noise = {center_temp.x, center_temp.y, depth, 0, 1, camera_n, get_m_sec()}; // 新しい位置を記録
...
#define arc_i 0.96 //
        ...
        else
        {
            ...
            //カメラヨーの補正
            auto camera_yaw_now = clamp(rdn((float)(bestMatch.x - temp_point.x) * (hfov_rgb / 640)), -rdn(1.0f), rdn(1.0f));
            camera_yaw_org = 0;                                                                  // camera_yaw_org_old * 0.8 + 0.2 * camera_yaw_now;                    // 0.2は補正係数
            camera_yaw_mutch = isWithinRange(abs(camera_yaw_org - camera_yaw_now), 0.0f, 0.05f); // マッチングの閾値
            // camera_yaw_org = 0;
...
#define roi_r true
    int findBall(Mat *img, bool *strike, Mat *mask_image, Mat *mask_image_batter, int ball_detection_phase, int dep_l_en_i, int dep_l_kin_i, int *i_ball) // 讀懃ｴ｢逕ｻ蜒上√せ繝医Λ繧､繧ｯ蛻､螳夂ｵ先棡・亥・蜉幢ｼ峨∬ｷ晞屬諠・ｱ逕ｻ蜒・
    {
        ...
        if (center_temp_old_f.x == 0 && center_temp_old_s.x != 0)
            center_temp_old = center_temp_old_s; // 前回のボール位置
        if (center_temp_old_f.x != 0 && center_temp_old_s.x == 0)
            center_temp_old = center_temp_old_f; // 前回のボール位置
        ...
        case far_away:
            ...
            roi_y2 = img->rows - roi_y1 - 120;
            break;
        case beyond_the_base:
            mini_ball_area = 16; //c_mini_ball_area;
            max_ball_area = 150;
            kahen_enkeido = c_enkeido - 0.1;
#if (roi_r) //
            if (center_temp_old.x != 0)
            {                                                                                          //
                roi_x1 = (int)clamp(center_temp_old.x - c_rect_x - bias_x, 0, img->cols - bias_x * 2); //
                // roi_y1 = (int)clamp(center_temp_old.y - c_rect_y - 0, 50, img->rows - bias_y);
                roi_y1 = 50;
                roi_x2 = (int)clamp(bias_x * 2, 0, bias_x * 2); //
                //roi_y2 = (int)clamp(bias_y + 20, 0, img->rows - roi_y1);
                roi_y2 = (int)(img->rows - roi_y1);
            }
            else
            {
                roi_x1 = c_no_noize_area; //192 + 60 = 252,640 -192 - 60 = 388
                roi_y1 = 50;
                roi_x2 = img->cols - c_no_noize_area * 2; // img->cols-roi_x1*2; //
    ...
    // LOG_DEBUG.printf(" ddd:%d,%d,%d,%d",img->cols,img->rows,c_rect_x,c_rect_y);
        ...
        *strike = false;
        ...
        if (pre_contours_ball.size() == 0)
        {
            LOG_DEBUG_IF(ball_detection_phase != far_away).printf("roi:(%d,%d,%d,%d),ball_count Zero !!! :%d,camera:#%d", roi_x1 + c_rect_x, roi_y1 + c_rect_y, roi_x2, roi_y2, pre_contours_ball.size(), isSecondCamera + 1);
            return ret;
        } // ボールの輪郭が検出されなかった場合
        ...
        {
            if (isWithinRange(contourArea(contour), 1, 500)) // 最小面積しきい値) //
            {
        ...
        if (isSecondCamera)
        {
            ...
            {
                if (isWithinRange(contourArea(contour), 500.0f, 1000000.0f)) // 最小面積しきい値) //
                {
        ...
        {
            LOG_DEBUG_IF(ball_detection_phase != far_away).printf("ball_count Zero !!! :%d,camera:#%d", contours_ball.size(), isSecondCamera + 1);
            return (shape_none); // ボールが検出されなかった場合
        }
        ...
        for (int i = 0; i < contours_size; i++)
        {
            ...
            bool ballOldAddMutch = isWithinRange(center_temp.x, center_temp_old.x - 12, center_temp_old.x + 12)     //
                                   && isWithinRange(center_temp.y, center_temp_old.y - 32, center_temp_old.y + 60); //

            if (checkBallNoise(center_temp, 0, &noise_table_no, int(isSecondCamera + 1), ball_detection_phase))
            {
                if (!test_measurement_mode && !ballOldAddMutch)
                {
                    ball_err = noise_check_err;
                    contin = true;
                    LOG_DEBUG_IF(!test_measurement_mode && ball_detection_phase != far_away).printf(" %d:noise err :%d,%d,noise_table_no:%d", i, contours_ball[i][0].x + c_rect_x, contours_ball[i][0].y + c_rect_y, noise_table_no);
                    continue;
                }
            }
            //}
...
contin0:
                    ...
                    if (contin && !ballOldAddMutch)
                    {
                        ...
                        LOG_DEBUG.printf("   %d:SKIP!!!(x:%.0f,y:%.0f,%s)", i, center_temp.x, center_temp.y, BallErrResultToString(ball_err));
                        continue;
                    }
                    ...
                    if (dep_cnt > 2)
                    {
                        ...
                        */
                        ...
                        {
                            if (last_dep_i != 0 && checkBallNoise(center_temp, last_dep_i, &noise_table_no, int(isSecondCamera + 1), ball_detection_phase) //
                                && !ballOldAddMutch                                                                                                        //
                        ...
                        // ベース外れ度合い
                        if (                                                                                                                            //
                            ball_detection_phase == far_away && !isWithinRange(ballCenter_cm_x, -70, 70)                                                //
                            || (ball_detection_phase == beyond_the_base && (!isWithinRange(ballCenter_cm_x, -65, 65)))                                  // && !isWithinRange(w_tmp, -8, 8)))  //
...
contin1:
                    if (contin)
                    {
                        ...
                        // スキップする際のログ出力
                        LOG_DEBUG_IF(!test_measurement_mode).printf("   %d:SKIP!!!(x:%.0f,y:%.0f,%d:%s)", i, center_temp.x, center_temp.y, ball_err, BallErrResultToString(ball_err));
                        continue;
                    }
                    else
                    {
                        ...
                        {
                            LOG_DEBUG_IF(!test_measurement_mode).printf("  %d:ZONE hazure w:%.1f(cm),h:%.1f(cm),%.1f(cm)", i, ballCenter_cm_x, ballCenter_cm_y, c_dis_cm(last_dep_i));
            ...
            {
                //if (isWithinRange(contours_ball[i][0].x + c_rect_x, 150, 450))
                LOG_DEBUG_IF(isWithinRange(area_ball, 1, c_max_ball_area) && (ball_detection_phase != far_away && ball_detection_phase != invalid) && !test_measurement_mode).printf("  %d:areaSize err:%.1f(%d,%d) camera:#%d", i, area_ball, contours_ball[i][0].x + c_rect_x, contours_ball[i][0].y, isSecondCamera + 1);
                // area_ball = 0;
            }
        }
        if (ret != ball_none)
        {
            ...
            LOG_DEBUG_IF(!test_measurement_mode).printf("*** %d:find ball (x,y):(%d,%d)***", ret_i, (int)center_temp.x, (int)center_temp.y);
            // break;
...
*/
    ...
    bool updateOldballPosition(int ball_detection_phase)
    {
        ...
        if (isSecondCamera)
        {
            center_temp_old_s = center;
            if (center_temp_old_f.x == 0)
                center_temp_old_f = center;
            LOG_DEBUG.printf(" center_temp_old_f (%.0f, %.0f)", center_temp_old_f.x, center_temp_old_f.y);
        }
        else
        {
            center_temp_old_f = center;
            if (center_temp_old_s.x == 0)
                center_temp_old_s = center;
