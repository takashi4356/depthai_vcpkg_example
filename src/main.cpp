...
// カラー画像取得スレッド
void img_key(std::shared_ptr<dai::Device> &device);
...
//キュー
//mono_read_f
std::shared_ptr<dai::MessageQueue> q_f; // NEW (v3)
std::shared_ptr<dai::MessageQueue> qRgb; // NEW (v3 に合わせた宣言、または v2 API の戻り値型)
std::shared_ptr<dai::Buffer> buffer2;; // V2->V3
//mono_read_s
std::shared_ptr<dai::MessageQueue> q_s; // NEW (v3)

// std::shared_ptr<dai::IMUData> imuData;
// std::shared_ptr<dai::DataOutputQueue> imuQueue;

//! DEPTHAI NODE 左モノカメラ
std::shared_ptr<dai::node::Camera> monoLeft_B;
//! DEPTHAI NODE 右モノカメラ
std::shared_ptr<dai::node::Camera> monoRight_B;
//! DEPTHAI NODE 深度
...
以上が、この選択されたコードの概要です。設定ファイルから値を読み込むための基本的な手順を示しています。*/
    ...
    std::shared_ptr<dai::Device> device_B = std::make_shared<dai::Device>(deviceInfo2);
#endif
...
#if (double_camera == true)
    dai::Pipeline pipeline_B(device_B);
    dai::PipelineImpl(pipeline_B, false);
#endif
    ...
    // monoLeftとmonoRightの定義
    if (p720p_on)
    {
    monoLeft = pipeline.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_B, std::make_pair(1280, 720), 143);
    monoRight = pipeline.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_C, std::make_pair(1280, 720), 143);
    }
    else
    {
    monoLeft = pipeline.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_B, std::make_pair(640, 400), 143);
    monoRight = pipeline.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_C, std::make_pair(640, 400), 143);
    }
    ...
    if (p720p_2_on)
    {
    monoLeft_B = pipeline_B.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_B, std::make_pair(1280, 720), 143);
    monoRight_B = pipeline_B.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_C, std::make_pair(1280, 720), 143);
    }
    else
    {
    monoLeft_B = pipeline_B.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_B, std::make_pair(640, 400), 143);
    monoRight_B = pipeline_B.create<dai::node::Camera>()->build(dai::CameraBoardSocket::CAM_C, std::make_pair(640, 400), 143);
    }
    ...
    */
    ...
    // manip_l->setMaxOutputFrameSize(monoRight->getResolutionHeight() * monoRight->getResolutionWidth() * 1); //V2->V3
    // manip_r->setMaxOutputFrameSize(monoRight->getResolutionHeight() * monoRight->getResolutionWidth() * 1); //V2->V3
    // Resize image
    ...
    // manip_l_B->setMaxOutputFrameSize(monoRight->getResolutionHeight() * monoRight->getResolutionWidth() * 1); //V2->V3
    // manip_r_B->setMaxOutputFrameSize(monoRight->getResolutionHeight() * monoRight->getResolutionWidth() * 1); //V2->V3
    // Resize image
    ...
    */
    ...
    camRgb->requestOutput((std::make_pair(640, 400)))->link(script->inputs["frames"]); //V3
...
#if (double_camera == true)
    // camera_B Linking
    monoLeft_B->requestOutput({ 256, 400 }, dai::ImgFrame::Type::NV12,
        dai::ImgResizeMode::CROP, 200)->link(manip_l_B->inputImage); //V3
    monoRight_B->requestOutput({ 256, 400 }, dai::ImgFrame::Type::NV12,
        dai::ImgResizeMode::CROP, 200)->link(manip_r_B->inputImage); //V3
    // monoLeft_B->out.link(manip_l_B->inputImage); // V2
    // monoRight_B->out.link(manip_r_B->inputImage); // V2
    /* V2->V3
    ...
    */
    ...
    /**********************************************************************************/
    //mono_read_f
    q_f = manip_hanten->out.createOutputQueue(); // NEW (v3)
    buffer2 = std::make_shared<dai::Buffer>(); // V2->V3
    ...
    //mono_read_s
    q_s = manip_hanten_B->out.createOutputQueue(); // NEW (v3)
...
void mono_read_f()
{
    ...
    while (end_flg_mono_read)
    {
        ...
        inDepth_f = q_f->tryGet<dai::ImgFrame>();
        if (inDepth_f)
        {
            if (p720p_on)
            {
...
void mono_read_s()
{
    ...
    while (end_flg_mono_read)
    {
        ...
        inDepth_s = q_s->tryGet<dai::ImgFrame>();
        if (inDepth_s)
        {
            if (p720p_2_on)
            {
 ...
 * @sa
 */
void img_key(std::shared_ptr<dai::Device> &device2)
{
