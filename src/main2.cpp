...
//! 距離画像スレッド動作
bool isActJudgment_main = true;
bool end_flg_Judgment_main_get = true;
bool end_flg_rgb_read = true;
bool end_flg_mono_read = true;
bool mono_read_lock = false;
bool rgb_read_lock = false;
bool end_flg_img_key = true;
bool rnd_filter_syori_flg = false;
...
#endif
void rgb_read(dai::Device &device);
#if (double_camera == true)
void mono_read(dai::Device &device, dai::Device &device_B);
#else
void mono_read(dai::Device &device);
#endif
void mono_read_B(dai::Device &device_B);
void img_key(dai::Device &device);
bool autoex_mono = false; // 自動露出（パフォーマンス低下）になっている
bool autoex_rgb = true;
bool cond_flg = true;
bool rgb_frame_taken = true;
bool rgb_frame_taken_img_key = false;  //カラー画像取得した
bool mono_frame_taken_img_key = false; //mono画像取得した
...
float c_zone_low_cm;
float c_zone_hi_cm;
int camera_sel = 0;    //有効カメラ（0：ダブル、１，２）
int strike_course = 0; //投球されたボールのコース（0:ど真ん中、1:高め,2:真ん中低め,3:左右真ん中以下低め）
...
Mat frame_mono_l;
Mat clip_img, clip_img_o, clip_img_o2;
Mat tmp_frame_depth, tmp_frame2_depth;
Mat ball_rireki = Mat::zeros(400, 640, CV_8UC1);
// Mat frame_confidenceMap;
Mat frame_rectifiedLeft;
// Mat ball_snap;
Mat resize_frame, tmp_frame, tmp_frame2, r_image_org, r_image_org_shrink;
std::shared_ptr<dai::ImgFrame> inRgb, inDepth, inDepth_B, inMono, inconfidenceMap, inrectifiedLeft;
bool JudghTrigger_sw = false;      // 判定トリガー
bool AutoJudgh_sw = false;         // 自動判定モード
bool AutoJudgh_sw_keyboad = false; // 自動判定モード
bool Judgment_start = false;       // 判定開始指示フラグ TRUE判定モードに変わる 判定モード開始されるとFALE
bool syuuryousyori = false;        // 終了処理の開始指示フラグ 各種フラグを初期化される
bool bat_flg = false;              // 打者検知済か
bool right_bat_f = false;
bool left_bat_f = false;
bool bat_pose_flg = false; // 打撃ポーズを取っているか
int bat_pose_flg_cnt = 0;
...
float fov_org;
std::chrono::time_point<std::chrono::steady_clock> ts_org;
...
float snap_speed = 0;
bool mono_read_check = true; //mono_read_errorのチェック是非
...
bool strike;
int ball_err_renzoku_cnt = 0; // ボール検知エラー連続カウント
int cntShinkou = 0;
float knee_keisuu;
Point2i center_last;
...
int knee_x, knee_y, shoulder_x, shoulder_y, hip_x, hip_y, wrist_x, wrist_y, elbow_x, elbow_y;
Point2i knee_point; // 膝の座標
Point2i shoulder_point;
Point2i hip_point;
Point2i wrist_point;
Point2i elbow_point;

int c_speed_lim_min; // 前フレームからのボール移動距離のしきい値（ｃｍ）。この値以下の場合はボール検知カウントしない
...
int tsuuka_filter; // 投球検知判定のしきい値（ｃｍ）。最初にと最後にボール検知した距離の差がこの値以下であれば投球としない
Base basez;
...
void play_sound_func_2(int n)
{
    ...
    // while(true)
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(1)); // 200msスリープ
        if (play_sound_f)
        {
            if (strike_last)
            {
...
以上が、この選択されたコードの概要です。設定ファイルから値を読み込むための基本的な手順を示しています。*/
    ...
    */
...
#endif
    {
        ...
        thread th_b(rgb_read, std::ref(device));
        thread th_d(mono_read, std::ref(device), std::ref(device_B));
        //thread th_e(mono_read_B, std::ref(device_B));
#else
        thread th_a(Judgment_main, std::ref(device));
        thread th_b(rgb_read, std::ref(device));
        thread th_d(mono_read, std::ref(device));
#endif
    ...
    */
    ...
    while (isActJudgment_main)
    {
        while (end_flg_Judgment_main_get)
        {
            ...
            if (mono_frame_taken_img_key)
            {
                //その後、std::lock_guard<std::mutex>を使用して、mtxという名前のmutexをロックし、スレッドセーフな処理を保証します。stateをtrueに設定します。
                std::lock_guard<std::mutex> lk(mtx);
                rnd_filter_syori_flg = true;
                mono_read_lock = true; // mono画像取得中フラグを立てる
                if (p720p_on && !second_camera_sht)
                    cv::divide(frame_mono_org, 2, frame_mono_org); // すべての画素値を2で割る
                if (p720p_2_on && second_camera_sht)
                    cv::divide(frame_mono_org, 2, frame_mono_org); // すべての画素値を2で割る
                frame_mono_org.copyTo(frame_mono);
                mono_frame_taken_img_key = false;                                                //mono画像取得済状態をリリース
                mono_read_lock = false; // mono画像取得中フラグを下ろす
                cnt_t = get_sec();
                fps_cnt++;
                second_camera_length_bias = second_camera_length_bias_org;
                ts_msec = std::chrono::duration_cast<std::chrono::milliseconds>(ts_org.time_since_epoch()).count();
            }
            else
            {
                continue;
            ...
            if (Judgment_mode || test_measurement_mode)
            {
                ...
                */
                ...
                if (basez.zone_flg) // ゾーン検出時のみ実施
                {
                    ...
                    int i = 0;
                    int kyori_filter = -15; //身体を検知しないように身体の厚み分は残しておいたほうがよい
                    ...
                    if (!frame_mono.empty() && !ground_mode && rnd_filter_f) // && !clip_img_o2.empty()) // セグメント例外の防止
                    {
                        ...
                        case far_away:
                            ...
                            {
                                filter_en = dep_l_en;
                                filter_kin = basez.c_dis_dep_i(dep_base_en + 60);
                            }
                            else
                            {
                        ...
                        for (int iy = 0; iy < temp_rows; iy++)
                        {
                            ...
                            for (int ix = 0; ix < frame_mono.cols; ix++)
                            {
                                ...
                                */
                                ...
                                if (!isWithinRange(src_org[ix], basez.c_dis_dep_i(160), basez.c_dis_dep_i(280)))
                                {
                                    if (!tmp_flg_a && !tmp_flg_b)
                                    {
                                        src_org[ix] = 0;
                    ...
                    // ボール検知エラー連続時はリセット
                    if (ball_detection_phase == beyond_the_base && codeBallPosition == ball_none) // && 300 < basez.ball_position_old.d) //!= base_shortly)
                    {
                        ball_err_renzoku_cnt++;
                        LOG_DEBUG.printf("ball_err_renzoku_cnt:%d,camera:#%d", ball_err_renzoku_cnt, +1);
                    }
                    ...
                    if ((codeBallPosition != ball_none) && !test_measurement_mode) // (ボールが検知された　＆　テスト測定モードでない)→ボール記録処理
                    {
                        ...
                        case far_away:
                            ...
                            else
                            {
                                ...
                                LOG_DEBUG.printf(" ボール記録なし! ball_shinkou:%d,cntShinkou:%d,last_dep_cm_old:%.1f,basez.ball_position.d:%.1f", ball_shinkou, cntShinkou, last_dep_cm_old, basez.ball_position.d);
                            }
                            break;
                        case beyond_the_base:
                            /* code */
                            ...
                            else
                            {
                                ...
                                {
                                    ball_detection_cnt = ball_detection_cnt_old; //ボール記録しない
                                    LOG_DEBUG.printf(" norecord! ball_shinkou:%d,cntShinkou:%d,last_dep_cm_old:%.1f,basez.ball_position.d:%.1f", ball_shinkou, cntShinkou, last_dep_cm_old, basez.ball_position.d);
                                }
                            }
                            break;
                        case on_the_base:
                            ...
                            )
                            {
                                ...
                                if (  //
                                    ( //
                                        isWithinRange(basez.ball_position.d, horizontalDistanceToBase_cm - 50, horizontalDistanceToBase_cm - 5)
...
*/
                        ...
                        if (ball_detection_cnt != ball_detection_cnt_old)
                        {
                            ...
                            if (ball[ball_dep].dep == ball_dep)
                            {
                                LOG_DEBUG_IF(!test_measurement_mode && ball[ball_dep].dep == ball_dep).printf(" ボールコース記録オーバーライト");
 ...
 */
#if (double_camera == true)
void mono_read(dai::Device &device3, dai::Device &device4)
#else
void mono_read(dai::Device &device3)
#endif
{
    bool cam_sw = true;
    int AB_change_cnt = 0;
    bool AB_change_flg = true;
    std::chrono::time_point<std::chrono::steady_clock> ts1, ts2;
    // 時間同期の設定
    std::chrono::milliseconds syncPeriod(1000); // 100msごとに同期
    int numSamples = 10;                        // 同期に使うサンプル数
    bool random = false;                        // ランダムな揺らぎはなし
    LOG_NONE.printf("start mono_read()");
    std::shared_ptr<dai::DataOutputQueue> q, q_B;
    // std::shared_ptr<dai::DataOutputQueue> q_mono;
    device3.setTimesync(syncPeriod, numSamples, random);
    q = device3.getOutputQueue("disparity", 32, true); //trueでバッファ少なすぎるとハングする
    // q_mono = device3.getOutputQueue("mono", 16, false);
    // リサイズ後の画像を格納する変数
    cv::Mat resizedImage;
    ...
    // cv::Mat frame_mono_f_tmp(400, 256, CV_8UC1, cv::Scalar(0)); // 初期値は黒（0）
    // cv::Mat frame_mono_s_tmp(400, 256, CV_8UC1, cv::Scalar(0)); // 初期値は黒（0）

    // 初期タイムスタンプを取得してオフセットを計算
    auto initialFrame1 = q->get<dai::ImgFrame>();
#if (double_camera == true)
    device4.setTimesync(syncPeriod, numSamples, random);
    // タイムスタンプをデバイスに設定
    q_B = device4.getOutputQueue("disparity_B", 32, true); //trueでバッファ少なすぎるとハングする
    auto initialFrame2 = q_B->get<dai::ImgFrame>();
    auto initialTs1 = initialFrame1->getTimestamp();
    auto initialTs2 = initialFrame2->getTimestamp();
    auto offset = initialTs1 - initialTs1;
    cv::Mat inDepth_getCvFrame_tmp;
    // inDepth_getCvFrame_tmp のサイズに合わせた ROI を作成（貼り付け先）
    // cv::Rect roi(64, 0, 128, 400);
...
#if (double_camera == true)
            device4.setTimesync(syncPeriod, numSamples, random);
            initialTs1 = initialFrame1->getTimestamp();
            initialTs2 = initialFrame2->getTimestamp();
            offset = initialTs1 - initialTs1;
#endif
            std::this_thread::sleep_for(std::chrono::milliseconds(50));
        }
        */

        //少しスリープしないとものカメラがハングする?
        ...
        //
        //tryブロック内では、mtx.lock()を使用してデプス画像の更新時にスレッドをロックします。q->tryGet<dai::ImgFrame>()を呼び出してデプス画像フレームを取得し、条件に応じてフレームを処理します。detNN == trueの場合は、getFrame()を使用してフレームを取得し、そうでない場合はgetCvFrame()を使用してOpenCV形式のフレームを取得し、fps_cntをインクリメントします。最後に、mtx.unlock()を呼び出してmutexのロックを解除します。
        // cond.notify_all();
        mtx.lock();           // depth画像更新時はスレッドロックする
        rgb_read_lock = true; // カラー画像取得済状態をロック
        inDepth_B = q_B->tryGet<dai::ImgFrame>();
        // 現在の時刻をタイムスタンプとして設定
        // inDepth_B->setTimestamp(std::chrono::steady_clock::now());
        // inMono = q_mono->tryGet<dai::ImgFrame>();
#if (double_camera == true)
    #if (false)
        if (inDepth && inDepth_B)
        {
            offset = inDepth->getTimestamp() - inDepth_B->getTimestamp();
        }
    #endif
#endif
#if (detNN == true)
        frame_mono_tmp = inDepth->getFrame();
#else
        if (inDepth_B)
        {
            frame_mono_s_tmp = inDepth_B->getCvFrame();
            //if (inMono)
            //    frame_mono_l = inMono->getCvFrame();
            // if (!inDepth && (Judgment_mode || test_measurement_mode))
            // {
            if (camera_sel == 2 || camera_sel == 0)
            {
                ...
                // frame_mono_org = frame_mono_s_tmp;
                // }
        ...
        {
            inDepth = q->tryGet<dai::ImgFrame>();
            if (inDepth)
            {
                if (p720p_on)
                {
                    inDepth_getCvFrame_tmp = inDepth->getCvFrame();
                    //    std::cout << "mono type: " << type2str(inDepth_getCvFrame_tmp.type()) << std::endl;
                    // ROI領域にコピー
                    inDepth_getCvFrame_tmp.copyTo(frame_mono_f_tmp(roi));
                }
                else
                {
                    frame_mono_f_tmp = inDepth->getCvFrame();
                }
                if (camera_sel == 1 || camera_sel == 0)
                {
                    basez.camera_high = basez.c_camera_high_no_base;
                    ts_org = inDepth->getTimestamp();
                    mono_frame_taken_img_key = true;
                    //fps_cnt++;
                    ...
                    // std::this_thread::sleep_for(std::chrono::milliseconds(10)); // 200msスリープ
                    // frame_mono_org = frame_mono_f_tmp;
                }
                if (!AB_change_flg)
                {
                    AB_change_flg = true;
        ...
        //if (AB_change_cnt > 3)
        //    printf("read_MONO error! %d\n", AB_change_cnt);
        basez.isSecondCamera = second_camera_sht;
        basez.is720p = (!second_camera_sht && p720p_on) || (second_camera_sht && p720p_2_on);
        if (AB_change_cnt > 1000 && camera_sel == 0)
        {
            LOG_DEBUG.printf("read_MONO error! %d", AB_change_flg);
            if (mono_read_check)
                exit_strike(true);
        }
        else
        {
            if (AB_change_cnt > 100)
                LOG_DEBUG_IF(AB_change_cnt % 10 == 0).printf(" warning! read_MONO change %d", AB_change_cnt);
        }
        ...
        /*
                ...
                {
                ts_msec = std::chrono::duration_cast<std::chrono::milliseconds>(ts_org.time_since_epoch()).count();
                if (!second_camera_sht) printf(" first_camera ");
                if(second_camera_sht) printf(" second_camera ");
                printf("ts_msec_old:%llu,ts_msec:%llu sa:%d\n", ts_msec_old, ts_msec, ts_msec - ts_msec_old);
                ts_msec_old = ts_msec;
                }
                */
        ...
        mtx.unlock();          // 排他制御解除
        rgb_read_lock = false; // カラー画像取得済状態をリリース
    }
    LOG_NONE.printf("end mono_read()");
}
/**
 * @brief 距離（mono）画像取得
 * @param
 ...
 * @sa
 */
void mono_read_B(dai::Device &device4)
{
    LOG_NONE.printf("start mono_read_B()");
    std::shared_ptr<dai::DataOutputQueue> q;
    q = device4.getOutputQueue("disparity_B", 4, false);
    while (end_flg_mono_read)
    {
        if (Judgment_mode)
            std::this_thread::sleep_for(std::chrono::milliseconds(200)); // 100msスリープ
        //tryブロック内では、mtx.lock()を使用してデプス画像の更新時にスレッドをロックします。q->tryGet<dai::ImgFrame>()を呼び出してデプス画像フレームを取得し、条件に応じてフレームを処理します。detNN == trueの場合は、getFrame()を使用してフレームを取得し、そうでない場合はgetCvFrame()を使用してOpenCV形式のフレームを取得し、fps_cntをインクリメントします。最後に、mtx.unlock()を呼び出してmutexのロックを解除します。
        try
        {
            // cond.notify_all();
            ...
            if (inDepth)
            {
                frame_mono_org = inDepth->getCvFrame();
                ts_org = inDepth->getTimestamp();
                mono_frame_taken_img_key = true;
                // fps_cnt++;
            }
        ...
        {
            LOG_ERROR.printf("Judgment_main()_runtime inDepth exeption %s", e.what());
            // continue;
        }
        catch (...) // その他の例外をキャッチ
        {
            LOG_ERROR.printf("Judgment_main() inDepth exeption");
        }
    }
    LOG_NONE.printf("end mono_read()");
}
 ...
 * @sa
 */
void rgb_read(dai::Device &device3)
{
    LOG_NONE.printf("start rgb_read()");
    ...
    // ctrl_Rgb.setAutoExposureLimit(1000);
    ctrl_Rgb.setAutoExposureEnable();
    // ctrl_Rgb.setAutoExposureLock(true);
    ...
    while (end_flg_rgb_read)
    {
        ...
        if (Judgment_mode && !snap_shot)
            std::this_thread::sleep_for(std::chrono::milliseconds(5)); // 100msスリープ
        std::this_thread::sleep_for(std::chrono::milliseconds(1));     //←モノカメラハング対応?
        // カラー画像取得
        if (!Judgment_mode || snap_shot)
        {
            try
            {
                if (snap_shot && !snap_shot_sumi)
                {
                    ...
                    for (int l = 0; l < 7; l++) //
                        //do
                    {
                        mono_read_lock = true; // mono画像取得済状態をロック
                        /*while (rgb_read_lock)
                        {
                        ...
                        mono_read_lock = false; // mono画像取得済状態をリリース
                        if (inRgb)
                        {
    ...
    while (end_flg_img_key)
    {
        ...
        if (!Judgment_mode)
        {
            ...
            if ((Judgment_start) && basez.zone_flg && !pr_flg)
            {
                    ...
                    && !isHorizontalError)
                {
                    ...
                    if (basez.zone_flg && !Judgment_mode)
                    {
                        ball_clear_sleep = false;
        ...
        if (Start_of_judgment)
        {
            ...
            else
            {
                ...
                Judgment_mode = false;
                ball_detection_phase = invalid;
                mono_read_check = false; //モノカメラ監視の対応
...
//撮影時間による直線回帰とハズレ値の除去
        ...
        if (!Judgment_mode)
        {
            ...
            if ((autoex_rgb && calibration_start_t - 1) < get_sec())
            {
                rgb_read_lock = true;
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                // ctrl_Rgb.setAutoExposureLock(true);
        ...
        */
        ...
        if (bin_img_f)
        {
            ...
            if (base_center.x != 0)
            {
                if (no_base_mode)
                {
                    circle(r_image_SARA, base_center, 20, Scalar(255, 215, 0), 1, LINE_AA, 0); // ベース中心表示
                }
                else
                {
                    circle(r_image_SARA, base_center, 3, Scalar(255, 215, 0), 1, LINE_AA, 0); // ベース中心表示
                }
            }
            //  ベース中心線描画
